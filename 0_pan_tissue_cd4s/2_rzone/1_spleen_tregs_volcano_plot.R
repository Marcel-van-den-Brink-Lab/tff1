...
assign_colors <- function(df, color_vector) {
  color_vector[df$'spleen_l' > 0 &
                 df$'spleen_p' < 10e-6 &
                 df$'spleen_l' > 0.5] <- 'orange'
  
  color_vector[df$'spleen_l' < 0 &
                 df$'spleen_p' < 10e-6 &
                 df$'spleen_l' < -0.5] <- 'darkgray'
  
  return(color_vector)
}

set_color_names <- function(df, color_vector) {
  names(color_vector) <- ifelse(df$'spleen_l' > 0 &
                                  df$'spleen_p' < 10e-6 &
                                  df$'spleen_l' > 0.5,
                                'Spleen',
                                ifelse(df$'spleen_l' < 0 &
                                         df$'spleen_p' < 10e-6 &
                                         df$'spleen_l' < -0.5,
                                       'Rest',
                                       'Not Significant'))
...
plot_volcano <- function(df, color_vector) {
  EnhancedVolcano(
    ...
    max.overlaps = 50,
    selectLab = c('Zfp36', 'Cd28', 'S100a10', 'Ighm', 
                  'Ctsw', 'Pmepa1', 'Tff1', 'Itgae', 'Il18r1', 'Ly6a', 'Vim')
  )
}

all <- read_excel("~/github/2_tff1/1_pyzone/1_outputs/1_degs/5_spleen_treg_vs_rest.xlsx")
...
all_plt <- all_plt +
  scale_colour_manual(
    values = c(Spleen = "orange", Rest = "darkgray"),
    breaks = c("Spleen", "Rest"),
    name = NULL
  ) +
...
out_path_pdf <- "~/github/2_tff1/1_pyzone/1_outputs/0_figures/volcanoplot_spleen_vs_rest.pdf"
