...
assign_colors <- function(df, color_vector) {
  color_vector[df$'skin_l' > 0 &
                 df$'skin_p' < 10e-6 &
                 df$'skin_l' > 0.5] <- 'blue'
  
  color_vector[df$'skin_l' < 0 &
                 df$'skin_p' < 10e-6 &
                 df$'skin_l' < -0.5] <- 'darkgray'
  
  return(color_vector)
}

set_color_names <- function(df, color_vector) {
  names(color_vector) <- ifelse(df$'skin_l' > 0 &
                                  df$'skin_p' < 10e-6 &
                                  df$'skin_l' > 0.5,
                                'Skin',
                                ifelse(df$'skin_l' < 0 &
...
plot_volcano <- function(df, color_vector) {
  EnhancedVolcano(
    ...
    #max.overlaps = 50,
    selectLab = c('Crip1', 'S100a6', 'Lmna', 'Prkcq', 'Gimap8', 'Itga4', 'Cnp', 'Pik3cd', 'Usp15', 'Dok2',
                  'Eif4b', 'Lrba', 'Sell')
  )
}

all <- read_excel("~/github/2_tff1/1_pyzone/1_outputs/1_degs/9_skin_treg_vs_rest.xlsx")
...
all_plt <- all_plt +
  scale_colour_manual(
    values = c(Skin = "blue", Rest = "darkgray"),
    breaks = c("Skin", "Rest"),
    name = NULL
  ) +
...
out_path_pdf <- "~/github/2_tff1/1_pyzone/1_outputs/0_figures/volcanoplot_skin_vs_rest.pdf"
