...
assign_colors <- function(df, color_vector) {
  color_vector[df$'0:Thymus_l' > 0 &
                 df$'0:Thymus_p' < 10e-6 &
                 df$'0:Thymus_l' > 0.5] <- 'red'
  
  color_vector[df$'0:Thymus_l' < 0 &
                 df$'0:Thymus_p' < 10e-6 &
                 df$'0:Thymus_l' < -0.5] <- 'darkgray'
  
  return(color_vector)
}

set_color_names <- function(df, color_vector) {
  names(color_vector) <- ifelse(df$'0:Thymus_l' > 0 &
                                  df$'0:Thymus_p' < 10e-6 &
                                  df$'0:Thymus_l' > 0.5,
                                'Thymus',
                                ifelse(df$'0:Thymus_l' < 0 &
                                         df$'0:Thymus_p' < 10e-6 &
                                         df$'0:Thymus_l' < -0.5,
                                       'Rest',
                                       'Not Significant'))
...
plot_volcano <- function(df, color_vector) {
  EnhancedVolcano(
    ...
    max.overlaps = 50,
    selectLab = c('Ctsw', 'Pmepa1', 'Itgae', 'Tff1', 'Ccr7', 'Il18r1', 'Ikzf4', 'Bach2', 'Nrp1', 'Tcf7',
                  'Srgn', 'S100a11', 'Tigit', 'Ccr2', 'Gzmb', 'Ikzf3', 'Batf')
  )
}

all <- read_excel("~/github/2_tff1/1_pyzone/1_outputs/1_degs/1_nonss_thymus_treg_vs_rest.xlsx")
...
all_plt <- all_plt +
  scale_colour_manual(
    values = c(Thymus = "red", Rest = "darkgray"),
    breaks = c("Thymus", "Rest"),
    name = NULL
  ) +
...
out_path_pdf <- "~/github/2_tff1/1_pyzone/1_outputs/0_figures/non_ss_volcanoplot_thymus_vs_rest.pdf"
